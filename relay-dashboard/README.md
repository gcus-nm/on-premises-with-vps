...
経路一覧は「すべて」「反映済み」「未反映」「削除済み」のタブで絞り込めます。
...
| 反映済み | TerraformとOCIリレー同期が両方成功した |
| 削除済み | OCIとリレーからの削除が両方成功した |

一度もApplyしていない作成待ち経路を削除すると、作成そのものを取り消します。
反映済み経路を削除すると削除待ちになり、「削除を取り消す」で元に戻せます。
削除待ちのままApplyが成功すると削除済みタブへ移り、設定内容と削除日時が履歴として
無期限に残ります。「履歴から消去」はOCIやリレーを変更せず、その履歴だけを完全に
消去します。

既存のバージョン1形式の`routes.json`は初回読込時に自動移行し、登録済み経路を
反映済みとして引き継ぎます。アップデート前に削除された経路は復元できません。
